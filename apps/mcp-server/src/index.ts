...
async function main() {
  ...
  const CHAIN_ID = parseInt(process.env.CHAIN_ID ?? '296', 10)

  // Initialize HCS audit trail
  const { initHcs } = await import('./hcs/client.js')
  ...
  const { registerAgent } = await import('./hcs/agent-identity.js')
